...
def editSheet(dicHelix,dicSheet,dicChain):
    sheetNo,chainID=validation.sheetNoValid_S(dicSheet)     #sheetNo, chainID

    newChainID=validation.chainValid(chainID,dicChain)      #new chain ID

    initSeqNum=validation.initSeqNumValid_S(sheetNo, dicHelix,dicSheet,newChainID,dicChain) #init seq number
    
    endSeqNum =validation.endSeqNumValid_S(sheetNo, dicHelix,dicSheet,newChainID,dicChain,initSeqNum) #end seq number
...
#updates main file list with changes   
...
def mainEditSheet(dicHelix,dicSheet,dicChain):
    ...
    line,sheetNo = editSheet(dicHelix,dicSheet,dicChain)    #gets update details

    strEdit = editSheetFileLst(line)                        # main file update
    if strEdit == None:
